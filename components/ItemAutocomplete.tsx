  ...
  return (
    <div className={`relative w-full ${className}`} ref={wrapperRef}>
      <div className="relative group">
        <input
          type="text"
          className="w-full p-2.5 pl-9 border-2 border-transparent bg-gray-50/50 hover:bg-white hover:border-indigo-100 rounded-xl focus:bg-white focus:border-indigo-500 focus:ring-4 ring-indigo-50 outline-none transition-all text-sm font-bold text-gray-900"
      ...
      {isOpen && (searchTerm || filteredItems.length > 0) && (
        <ul className="absolute z-[100] w-full mt-2 bg-white border border-indigo-50 rounded-2xl shadow-2xl max-h-72 overflow-auto py-2 animate-in fade-in slide-in-from-top-2 duration-200">
            ...
            return (
              <li 
                ...
                className={`px-4 py-3 cursor-pointer flex justify-between items-center transition-all border-l-4
                  ${idx === highlightedIndex ? 'bg-indigo-50 border-indigo-500' : 'hover:bg-gray-50 border-transparent'}
                `} 
          ...
          {filteredItems.length === 0 && (
            <li className="px-6 py-10 text-center text-gray-400">
                ...
                <p className="text-xs font-bold uppercase tracking-widest italic">Item not in Master</p>
            </li>
          )}
        </ul>
      )}
